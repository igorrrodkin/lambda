...
function jsonify(jsonList) {
  const result = [];
  const names = jsonList.map((item) => item.user.name);
  const namesUnique = Array.from(new Set(names));
  for (let i = 0; i < namesUnique.length; i++) {
    const datesArr = [];

    const filtered = jsonList.filter(
      (item) => item.user.name == namesUnique[i]
    );
    filtered.forEach((item) =>
      datesArr.push({ startDate: item.startDate, endDate: item.endDate })
    );
    result.push({ name: namesUnique[i], weekendDays: datesArr });
  }
  return result;
}
